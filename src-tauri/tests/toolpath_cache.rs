...
fn round_trip_preserves_toolpath_and_not_stale() {
    ...
    let snap = get_project_snapshot_inner(&loaded_lock, false, false).expect("snapshot ok");
...
fn stale_after_param_change() {
    ...
    let snap2 = get_project_snapshot_inner(&loaded_lock, false, false).expect("snapshot ok");
    assert!(
        snap2
            .operations
            .iter()
