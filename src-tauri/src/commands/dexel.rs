...
use crate::dexel::{
    clearance_for_tool, flat_endmill_clearance, toolpath_to_segments, DexelGrid, MotionSegment,
};
...
// ── get_demo_simulation_mesh ─────────────────────────────────────────────────

/// Build the motion segments for the built-in demo scenario.
///
/// Two-level stepped pocket on a 100 × 100 × 20 mm block:
///
/// - **Outer shelf** (Z = 15 mm, 5 mm deep): rasters the full [5, 95] × [5, 95] area.
/// - **Inner pocket** (Z = 10 mm, 5 mm deeper): rasters the [25, 75] × [25, 75] centre.
///
/// Tool: 10 mm flat endmill (5 mm radius).  Stepover: 8 mm.
fn build_demo_segments() -> Vec<MotionSegment> {
    const TOOL_R: f64 = 5.0;
    const STEPOVER: f64 = 8.0;

    let mut segments = Vec::new();

    // Helper: append a boustrophedon raster between (x_lo, x_hi) at fixed y values
    // from y_lo to y_hi, all at constant z.
    let mut raster = |x_lo: f64, x_hi: f64, y_lo: f64, y_hi: f64, z: f64| {
        let mut y = y_lo;
        let mut forward = true;
        while y <= y_hi + 1e-9 {
            let (x0, x1) = if forward { (x_lo, x_hi) } else { (x_hi, x_lo) };
    ...
    // Outer shelf: X ∈ [5, 95] shrunk by tool radius on each side → [10, 90].
    raster(
        5.0 + TOOL_R,
        95.0 - TOOL_R,
        5.0 + TOOL_R,
    ...
    // Inner pocket: X ∈ [25, 75] shrunk by tool radius → [30, 70].
    raster(
        25.0 + TOOL_R,
        75.0 - TOOL_R,
        25.0 + TOOL_R,
...
/// Testable inner logic for [`get_demo_simulation_mesh`].
///
/// Runs the built-in demo scenario without touching project state: a 100 × 100 × 20 mm
/// stock block machined with a two-level stepped pocket using a 10 mm flat endmill.
pub(crate) fn get_demo_simulation_mesh_inner(resolution: f64) -> Result<MeshData, AppError> {
    if !(0.01..=5.0).contains(&resolution) {
        return Err(AppError::InvalidInput(
            "resolution must be between 0.01 and 5.0".into(),
        ));
    ...
    let segments = build_demo_segments();

    let mut grid = DexelGrid::from_stock(&stock, resolution);
    grid.apply_segments(&segments, tool_radius, &z_clearance);

    Ok(grid.extract_mesh())
}

/// Run the built-in demo simulation on a synthetic stock block and return the mesh.
///
/// Does not read or modify project state — safe to call on an empty or open project.
#[tauri::command]
pub async fn get_demo_simulation_mesh(resolution: f64) -> Result<MeshData, AppError> {
    get_demo_simulation_mesh_inner(resolution)
}
...
mod tests {
    ...
    // ── get_demo_simulation_mesh tests ──────────────────────────────────

    #[test]
    fn demo_invalid_resolution_too_low() {
        assert!(matches!(
            get_demo_simulation_mesh_inner(0.001),
            Err(AppError::InvalidInput(_))
        ));
    }

    #[test]
    fn demo_invalid_resolution_too_high() {
        assert!(matches!(
            get_demo_simulation_mesh_inner(10.0),
            Err(AppError::InvalidInput(_))
        ));
    }

    #[test]
    fn demo_produces_valid_mesh_at_coarse_resolution() {
        let mesh = get_demo_simulation_mesh_inner(2.0).unwrap();
        assert_mesh_valid(&mesh);
    }

    #[test]
    fn demo_produces_valid_mesh_at_default_resolution() {
        let mesh = get_demo_simulation_mesh_inner(0.5).unwrap();
        assert_mesh_valid(&mesh);
    }

    #[test]
    fn demo_removes_material_from_stock() {
        // Compare volume at two resolutions: both should be less than pristine stock.
        // Pristine 100×100×20 stock = 200_000 mm³.
        let mesh_coarse = get_demo_simulation_mesh_inner(2.0).unwrap();
        // The demo cuts two pockets so vertex count should be substantial.
        assert!(
            mesh_coarse.vertices.len() > 0,
            "demo mesh should have vertices"
        );
        // And the mesh should have more triangles than an untouched box
        // (untouched 100×100×20 at 2mm res = 50×50 grid; top+bottom=5000 tris,
        // walls = 4×50×2=400 → 5400 tris. After cutting, interior steps add more).
        let untouched_tri_count = {
            let stock = StockDefinition::Box(BoxDimensions {
                origin: Vec3 {
                    x: 0.0,
                    y: 0.0,
            ...
            let grid = DexelGrid::from_stock(&stock, 2.0);
            grid.extract_mesh().indices.len() / 3
        };
        let demo_tri_count = mesh_coarse.indices.len() / 3;
        assert!(
            demo_tri_count > untouched_tri_count,
            "cut workpiece ({demo_tri_count} tris) should have more triangles than \
             untouched stock ({untouched_tri_count} tris) due to step walls"
        );
    }

    #[test]
    fn demo_inner_pocket_is_deeper_than_outer_shelf() {
        // At resolution 1mm the grid is accurate enough to sample specific cells.
        // The outer shelf cuts to Z=15 (5mm deep from top of 20mm stock).
        // The inner pocket cuts to Z=10 (10mm deep).
        // Sample a cell at the stock centre (50, 50) — inside the inner pocket →
        // top should be ~10.  Sample a cell near the outer shelf but outside the
        // inner pocket, e.g. (12, 12) → top should be ~15.
        let stock = StockDefinition::Box(BoxDimensions {
            origin: Vec3 {
                x: 0.0,
                y: 0.0,
        ...
        // Centre of inner pocket — should be cut to Z=10.
        let inner_top = grid
            .height_at(50.0, 50.0)
            .expect("centre cell should have material");
        assert!(
            (inner_top - 10.0).abs() < 1.5,
            "inner pocket centre top Z should be ~10, got {inner_top}"
        );

        // Near outer shelf (x=12, y=12) — should be cut to Z=15 but not deeper.
        let outer_top = grid
            .height_at(12.0, 12.0)
            .expect("outer shelf cell should have material");
        assert!(
            (outer_top - 15.0).abs() < 1.5,
            "outer shelf top Z should be ~15, got {outer_top}"
        );

        // Untouched corner (x=1, y=1) — should still be at Z=20.
        let corner_top = grid
            .height_at(1.0, 1.0)
            .expect("corner cell should have material");
        assert!(
            (corner_top - 20.0).abs() < 1.5,
            "untouched corner top Z should be ~20, got {corner_top}"
        );
    }
}
